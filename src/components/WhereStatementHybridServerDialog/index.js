...
export const WhereStatementHybridServerDialog = ({
  open,
  toggleDialog,
  fieldName,
  categoryName,
  ...
  useEffect(()=>{
    ...
    if(typeWhereDefinition=="previous"){
      let p=conditionsWhere?.[categoryName]?.[fieldName]
      if(p!==undefined){
      let sfcw=Object.keys(conditionsWhere?.[categoryName]?.[fieldName]).filter(x=>{
  ...
  const displayAllWhereClausesSelect=()=>{
    ...
    if(typeof conditionsWhere=="object"){
      let fields=Object.keys(conditionsWhere).map(u=>{
    ...
    >
      ...
      <div style={{display:"flex",flex:1,backgroundColor:"brown",color:"white",height:"20px",padding:0,margin:0,marginRight:0,marginBottom:"5px"}}>
        <select  onChange={(e)=>{
          ...
          setSelectedRule("")
          displayRulesCombo(e.target.value,e.target.value)
        }}
          value={selectedField}
          style={{outline:"none",margin:0,padding:0,alignItems:"center",width:"100%",backgroundColor:"brown",color:"white",border:"none",height:"20px",lineHeight:"20px"}}>
            <option value="">Select Field33</option>
